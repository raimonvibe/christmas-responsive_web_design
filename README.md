...
/assets/
   ...
   /css/christmas.css   ← lights, snow, Holly palette, theme toggle
   /js/tour.js          ← guided tour steps + engine
   /js/theme-boot.js    ← applies saved theme before first paint
   /js/theme.js         ← theme toggle + string-light injection
   /js/snowflakes.js    ← hover snowflakes on festive themes
   /images/christmas-lights.svg
...
Seasonal styling is ported from [christmas-responsive_web_design](https://github.com/raimonvibe/christmas-responsive_web_design): blinking string lights across the top, and snowflakes that fall when you hover buttons or social icons.

A **theme** button sits above the Tour control and cycles three looks. The choice is stored in `localStorage` under `rv-theme`.

* **Classic** (default) — the holiday decorations from the source repo, on the existing purple Hyperspace palette.
* **Holly** — the same lights and snow, with a forest-green / berry-red / gold palette.
* **Default** — the year-round Ethereal look, with lights and snow turned off.

`prefers-reduced-motion` disables the bulb blink and the snowflakes.
